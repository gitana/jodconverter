...
class ManagedOfficeProcess {
  ...
  public ManagedOfficeProcess(ManagedOfficeProcessSettings settings) throws OfficeException {
    ...
    this.process = new OfficeProcess(settings.getOfficeHome(), settings.getUnoUrl(), settings.getRunAsArgs(),
        ...
        protected void attempt() throws TemporaryException, Exception {
          try {
            ...
            if (exitCode == null) {
              ...
              throw new OfficeException("office process died with exit code " + exitCode, connectException);
            }
          }
        }
  ...
  private void doStopProcess() {
    try {
      XDesktop desktop = OfficeUtils.cast(XDesktop.class, this.connection.getService(OfficeUtils.SERVICE_DESKTOP));
      desktop.terminate();
